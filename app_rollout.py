...
def main():
    ...
    logging.debug(f"{id=}, {appname=}, {appversion=}, {bundleidentifier=}")
    if args.appversion:
        appversion = args.appversion

    if id:
        appDetails = get_mobile_app_details(id)

    if appname and appversion:
        deployGroup = get_mobile_device_group(
            f"{appname} Deployment Group {appversion}"
        )
        logging.debug(f"{deployGroup=}")

    if type(deployGroup) == int:
        logging.debug("Create a new mobile group for deployment")
    else:
        logging.debug("Continue with the deployment")
